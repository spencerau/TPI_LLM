...
def init_rag():
    try:
        with st.spinner("Initializing RAG system..."):
            ...
            st.session_state.session_id = str(st.session_state.chat_session.session_id)
...
def get_collection_stats():
    ...
    for name, collection_name in collections.items():
        try:
            scroll_result = st.session_state.chat_session.rag.client.scroll(
...
def run_query(query: str, use_streaming: bool = True):
    ...
    try:
        result = st.session_state.chat_session.chat(
            query,
...
def search_collection(query: str, collection_name: str, top_k: int = 5):
    ...
    try:
        results = st.session_state.chat_session.rag.search_collection(
            query=query,
            collection_name=collection_name,
            top_k=top_k
        )
...
def render_info_tab():
    ...
    with col1:
        ...
        st.json({
            ...
            "postgres_host": config.get('postgresql', {}).get('host', 'N/A'),
        })
